...
async function load(){
  ...
  const scheduleRows=ss.map(s=>({...s,calendar_rule:s.config?.calendar?`${s.config.calendar.local_time} ${s.config.calendar.timezone} · ${s.config.calendar.weekdays.join(',')}`:''}));
  document.querySelector('#schedules').innerHTML=table(scheduleRows,['id','schedule_kind','enabled','calendar_rule','every_seconds','next_run_local','next_run_at','last_scheduled_for','last_triggered_at','last_late_by_seconds','last_state','last_error']);
